...
$MySql="SELECT SelfControl FROM Personaggio  WHERE Userid='$Userid' ";
$Result=mysql_query($MySql);
$res=mysql_fetch_array($Result);
$SelfControl=$res['SelfControl'];


if ( $SelfControl < 5 && $px >= 2*$SelfControl ) {

	$MySql="UPDATE Personaggio SET SelfControl=SelfControl+1 WHERE Userid='$Userid'";
	$Result=mysql_query($MySql);

	$pxspesi=2*$SelfControl;
	$Dati="[SelfControl] SelfControl ".$SelfControl." => ".($SelfControl+1);
